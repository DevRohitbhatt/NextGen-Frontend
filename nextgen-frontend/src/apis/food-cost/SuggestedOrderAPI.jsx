...
export const SuggestedOrderAPI = {
	...
	getPurchaseOrderDetails: async function (companyID, purchaseOrderID, cancel = false) {
		const response = await api.request({
			...
			signal: cancel
				? cancelApiObject[this.getPurchaseOrderDetails.name].handleRequestCancellation().signal
				: undefined,
		});
		return response.data;
	},
	...
	submit: async function (data, cancel = false) {
		const response = await api.request({
			method: 'POST',
			url: `/api/suggestedOrder/SubmitSuggestedOrder?companyID=${data.companyID}`,
			data,
