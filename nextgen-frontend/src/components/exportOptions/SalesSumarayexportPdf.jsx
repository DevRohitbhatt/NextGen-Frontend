...
import { formattingData, valueFormatewithoutDecimal } from '../../functions/formatingCurrency';
...
const generateSalesSummaryPDF = (data) => {
	...
	const formatPercent = (value) => (value !== undefined && value !== null ? `${(value * 100).toFixed(2)}%` : '0%');
	...
	const grossSalesSection = {
		table: {
			...
			body: [
				[{ text: 'Gross Sales', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.salesGross)],
				[{ text: 'Net Sales', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.salesNet)],
				[{ text: 'Tax', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.salesTax)],
				[{ text: 'Comps', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.comps)],
				[{ text: 'Promotions', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.promo)],
				[{ text: 'Refunds', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.refunds)],
				[{ text: 'Voids', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.voids)],
				[
					{ text: 'Sales Non Cash', style: 'tableHeader' },
					formattingData(data.viewActivityDaily[0]?.planSalesNet),
				],
				[{ text: 'Deposits', style: 'tableHeader' }, formattingData(data.viewActivityDaily[0]?.deposits)],
				[
					{ text: 'Cash OverShort', style: 'tableHeader' },
					formattingData(data.viewActivityDaily[0]?.cashOverShort),
				],
				[{ text: 'Order Count', style: 'tableHeader' }, data.viewActivityDaily[0]?.transactions || '0'],
				[{ text: 'Covers', style: 'tableHeader' }, data.viewActivityDaily[0]?.nrsTotalOpen || '0'],
				[{ text: 'Order Average', style: 'tableHeader' }, data.viewActivityDaily[0]?.orderAverage],
				[
					{ text: 'Labor Cost', style: 'tableHeader' },
					formattingData(data.viewActivityDaily[0]?.laborVariable + data.viewActivityDaily[0]?.laborSalary),
				],
				[{ text: 'Labor Hours', style: 'tableHeader' }, `${data.viewActivityDaily[0]?.laborHours}`],
				[
					{ text: 'Labor Percent', style: 'tableHeader' },
					`${(
						((data.viewActivityDaily[0]?.laborVariable + data.viewActivityDaily[0]?.laborSalary) /
		...
		table: {
			...
			body: [
				...
				[
					...
					formattingData(data.categorySummary.reduce((sum, item) => sum + item.salesNet, 0)),
					'',
				],
			],
		},
		...
		table: {
			...
			body: [
				...
				[
					...
					formattingData(data.paymentsSummary.reduce((sum, item) => sum + item.amount, 0)),
					formattingData(data.paymentsSummary.reduce((sum, item) => sum + item.tip, 0)),
					formattingData(data.paymentsSummary.reduce((sum, item) => sum + item.amount + item.tip, 0)),
					'',
				],
			],
		},
			...
			body: [
				...
				[
					...
					valueFormatewithoutDecimal(
						data.discountSummary.reduce((sum, item) => sum + item.quantityTickets, 0)
					),
					formattingData(data.discountSummary.reduce((sum, item) => sum + item.amountDiscount, 0)),
					'',
				],
			],
		},
