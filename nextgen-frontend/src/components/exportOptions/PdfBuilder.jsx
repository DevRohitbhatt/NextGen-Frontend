...
const rgbToHex = (rgb) => {
  // If RGB is an array, extract r, g, b values
  if (Array.isArray(rgb)) {
    const [r, g, b] = rgb;
    return `#${toHex(r)}${toHex(g)}${toHex(b)}`;
  }

  // If the input is a string like 'rgb(255, 0, 0)'
  ...
  return rgb; // If not in valid RGB or RGBA format, return the original string
};
      ...
      body: [
        ...
        ...tableInfo.data.rows.map((row) =>
          row.map((cell, index) => ({
            text: getCellValue(
              cell,
              tableInfo.dataTypes ? tableInfo.dataTypes[index] : ""
            ),
            style: "tableCell",
            fillColor: cell.color ? rgbToHex(cell.color) : null,
          }))
        ),
      ],
    },
