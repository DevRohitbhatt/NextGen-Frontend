...
const ExportButton = ({ isInclude, handleClick, iconName, title, className }) => {
	return (
		<div
			className={`
				${className}
				p-2 border-2 flex justify-center items-center border-solid w-[60px] h-[60px] border-[var(--tw-primary)] text-4xl rounded-full hover:bg-[var(--tw-primary)] text-[var(--tw-primary)] hover:text-white cursor-pointer ${
...
export default function ExportOptions(props) {
	const buttonsConfig = [
		...
		},
		{
			...
			icon: <FaFileExcel />,
			title: props.excelTitle || 'Export to Excel',
		...
		},
		{
			...
			icon: <FaFileCsv />,
			title: props.csvTitle || 'Export to CSV',
		...
		},
		{
			...
			icon: <FaFilePdf />,
			title: props.pdfTitle || 'Export to PDF',
		...
		},
		{
			...
			title: props.submitTitle || 'Submit',
		...
		},
		{
			...
			icon: <MdLibraryAdd />,
			title: props.addTitle || 'Create New',
