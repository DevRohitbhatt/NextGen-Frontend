  ...
  const handleMultipleUnitSelection = (id, name, isArea) => {
    ...
    if (isArea && area) {
      ...
      const newSelectedUnits = selectedUnits.some(unit => unit.id === id)
        ? selectedUnits.filter((unit) => !areaUnits.some((areaUnit) => areaUnit.id === unit.id) && id !== unit.id)
        : [
            ...selectedUnits.filter((unit) => !areaUnits.some((areaUnit) => areaUnit.id === unit.id)),
            { id, name, isArea },
            ...areaUnits.filter((unit) => !selectedUnits.some((selectedUnit) => selectedUnit.id === unit.id))
          ];
