...
const InventoryTransferReport = () => {
  ...
  const [inventoryTransferReportData, setInventoryTransferReportData] =
    useState([]);
  ...
  const fetchInventoryTransferReport = async () => {
    try {
      ...
      (await result?.data?.length) &&
        result?.data.forEach(
          (item) => (item.transferValue = formattingData(item.transferValue))
        );
      ...
      <div className='w-[98%] mx-auto'>
        ...
        <header className='optionsBar flex justify-between items-center mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
          <div className='flex items-center'>
            ...
            <Run fetchData={fetchInventoryTransferReport} />
          </div>
          <div>
            <ExportOptions
              includePDF={true}
