...
const tooltips = {
	forecastedDate: 'Forecasted Sales dates selected for this order',
	forecastedAmt:
	...
	itemRef: 'Vendor reference #.',
	itemOrderUnit: 'Vendor order unit i.e. case, box, etc.',
	packSize: 'Vendor packaging. How the item is packaged within the Vendor Order Unit.',
	currentLastPrice: 'Integrated Vendor = Current vendor catalog pricing. NON-Integrated Vendor = last invoice price.',
	safetyFactor: 'Individual line item Safety Factor can be selected if different from the Default Safety Factor.',
	suggestedQty:
	...
	onHand: 'Physical count of product on hand when creating your order. The On Hand value entered is deducted from the Suggested QTY to calculate the Order Amount.',
	orderLimits:
	...
	extendedPrice: 'Order Amount * Current/Last Price.',
};
...
const suggestedTableStructure = {
	...
	columnWidth: ['160px', '160px', '70px', '80px', '70px', '145px', '120px', '80px', '80px', '80px'],
	rows: [],
	headerTooltips: [
		'',
		tooltips.itemDescription,
		tooltips.itemRef,
		tooltips.itemOrderUnit,
		tooltips.packSize,
		tooltips.currentLastPrice,
		tooltips.safetyFactor,
		tooltips.suggestedQty,
		tooltips.onHand,
		tooltips.orderLimits,
		tooltips.orderAmount,
		tooltips.extendedPrice,
	],
	toolTipDirection: [
		'',
		'right',
...
export default function SuggestedOrder() {
	...
	const [vendorIsIntegrated, setVendorIsIntegrated] = useState(false);
	...
	const [forecastTable, setForecastTable] = useState({
		...
		rows: [],
		width: '50%',
		headerTooltips: [tooltips.forecastedDate, tooltips.forecastedAmt, ''],
	...
	const [defaultSafetyFactorTable, setDefaultSafetyFactorTable] = useState({
		...
		width: '20%',
		height: '40%',
		headerTooltips: [tooltips.defaultSafetyFactor],
	...
	useEffect(() => {
		...
		if (unit && vendorID && vendorName && dates) {
			...
			fillAddItemModalTable(companyID, unit, selectedVendor, dates[0], dates[1], suggestedOrderID);
	...
	const getOrderItem = (companyID, unitID, selectedVendor, orderFromDate, orderToDate, suggestedOrderId) => {
		...
		SuggestedOrderAPI.getOrderItem(
			...
			.then((response) => {
				...
				setVendorIsIntegrated(response.data.vendorIsIntegrated);
	...
	const fillAddItemModalTable = (companyID, unitID, vendorID, fromDate, toDate) => {
		SuggestedOrderAPI.getOrderItem(companyID, unitID, vendorID, formatDate(fromDate), formatDate(toDate), 0, -1)
			.then((response) => {
				const itemData = response.data.suggestedOrderDetails.flatMap((node) => {
					...
					return node.suggestedOrderItem.map((item) => {
						return {
							...
							equivalentToQSRInventoryItemID: item.equivalentToQSRInventoryItemID,
							equivalentCaseFactor: item.equivalentCaseFactor,
							invItemAvgSalesYieldPerMainUOM: item.invItemAvgSalesYieldPerMainUOM,
	...
	const checkIfItemMatchesSearch = (searchValue, item) => {
		...
		item.vendorItems.forEach((vendorItem) => {
			if (vendorItem.description.toLowerCase().includes(searchValue.toLowerCase())) {
				match = true;
			}
		});
		item.invItemDescription.toLowerCase().includes(searchValue.toLowerCase()) && (match = true);
		return match;
	};
	...
	const getOrderLimits = async (companyID, unitID) => {
		try {
			const getData = {
				url: 'getInventoryItemsOrderLimits',
				urlParams: {
					companyID: companyID,
					unitID: unitID,
				},
	...
	useEffect(() => {
		if (forecastTable.rows.length > 0 && suggestedTable.rows.length > 0) {
			const newSuggestedOrder = SuggestedOrderFunctions.calculateSuggestedQuantities(
	...
	const handleCSVClick = () => {
		...
		const csvDataString = columnHeaders.join(',') + '\n' + data.map((row) => row.join(',')).join('\n');
	...
	const handleTableCellChange = (e, row, columnName, tableName) => {
		...
		switch (tableName) {
			...
			case 'Forecast': {
				const updatedForecastData = SuggestedOrderFunctions.handleForecastChange(
					e,
					row,
					columnName,
					forecastTable,
	...
	return (
		<div className='w-[85%] mx-auto'>
			...
			<div className='my-4 text-2xl leading-tight text-left pageTitle'>Suggested Order</div>
			...
			<div className=' optionsBar flex justify-between mb-10 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
				...
				<SubmitPurchaseOrderModal
					...
					vendorIsIntegrated={vendorIsIntegrated}
					vendorName={selectedVendorName}
				/>
			</div>

			{isLoading ? (
				<div className='w-full mx-auto text-2xl text-center'>Loading...</div>
			) : isError ? (
				<div className='w-full mx-auto text-2xl text-center'>{errorMessage}</div>
			) : (
				<>
					<MinimizableContainer title={() => <div>Sales Forecast</div>}>
