...
const SuggestedOrderList = () => {
	...
	// This function is responsible for fetching the initial data required for the component.
	// It sets the isLoading state to true, then uses Promise.all to fetch units, vendors, and suggested orders concurrently.
	// Once all the data is fetched, it sets the isLoading state to false.
	...
	const fetchSuggestedOrders = async (companyID, alignmentID, memberID, vendorID, fromDate, toDate) => {
		try {
			setIsError(false);
			const getData = {
				url: 'orderList',
			...
			{isLoading ? (
				...
				<Styled.OptionsRow>
					...
					<Styled.VendorOrdersContainer>
						<Table data={filteredOrders?.data} headers={headers} onRowClick={handleRowItemClick} />
					</Styled.VendorOrdersContainer>
				</Styled.OptionsRow>
			)}
			<UnitModal
