	...
	const memoizedColumns = useMemo(
		() => [
			...
			columnHelper.accessor('date', {
				id: 'date',
				header: 'Date',
				cell: ({ getValue, row }) => (row.getCanExpand() ? '' : dateFormat(getValue(), 'mm/dd/yyyy')),
				dataType: 'date',
				filterFn: 'arrIncludesSome',
				size: 100,
			}),
			...
			columnHelper.accessor('quantity', {
				...
				cell: ({ getValue }) => <div className='text-center'>{getValue().toFixed(0) ?? 0}</div>,
				dataType: 'number',
				filterFn: 'weakEquals',
				size: 100,
				footer: ({ table }) => (
			...
			columnHelper.accessor('price', {
				id: 'price',
				header: 'Item Price',
				cell: ({ getValue, row }) => (getValue() && !row.getCanExpand() ? `$${getValue().toFixed(2)}` : ''),
				dataType: 'number',
				filterFn: 'weakEquals',
				size: 100,
			}),
			...
			columnHelper.accessor('extPrice', {
				id: 'extPrice',
				header: 'Item Total',
				cell: ({ getValue }) => (getValue() ? `${formattingData(getValue())}` : '$0.00'),
				footer: ({ table }) => {
	...
	return (
		<>
			<div className='w-[98%] mx-auto pageContainer'>
				...
				/>
				<h2 className='my-2 text-[18px] leading-tight text-left pageTitle'>Purchase Analysis</h2>
