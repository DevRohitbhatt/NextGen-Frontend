...
const CountsheetDesigner = () => {
		...
		return {
			...
			rows: data.map((row) =>
				columns.slice(1).map((column) => ({
					value: column.id === 'lineItemCost' ? formattingData(row[column.id]) : row[column.id] || '0 ',
					cellType: '',
		...
		const data = countsheetDetails.map((row) => ({
			...
			data: row.subRows.map((subRow) =>
				columns
					.slice(1)
					.map((column) =>
						column.id === 'lineItemCost' ? formattingData(subRow[column.id]) : subRow[column.id] || '0 '
					)
			),
		}));
	...
	return (
		<div className='w-[98%] mx-auto'>
			<h2 className='my-2 text-2xl leading-tight text-left pageTitle text-[18px]'>
				{`${countsheet?.name} --
				${countTypeMap[countsheet?.countType] || ''} Countsheet`}
			</h2>
			<header className='optionsBar flex justify-between items-center mb-2 rounded-2xl px-4 py-1 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
				<div className='flex items-center gap-1'>
					...
					<div
						className='flex items-center justify-between mt-8  px-6 py-2 text-center capitalize border-2 border-solid cursor-pointer text-nowrap rounded-3xl hover:border-[var(--tw-primary)] active:border-[var(--tw-primary)]'
					...
					<div className='pt-2 text-center border-t'>
						...
						<div className='flex items-center justify-center my-2'>
							...
							<div className='flex flex-col items-center'>
								...
								<p className='text-sm font-medium'>SAUCE SRIRACHA 20 OZ BTL - I=A</p>
