...
const tooltips = {
  ...
  usedEstimate:
    "Calculated average use per week. Amounts may vary depending on the selected Usage Estimation Model.",
  salesYieldWeeklyAverage: "Average Usage % * Estimated Weekly Sales Average",
  inventoryWeeksOnHandNow:
    "An estimation of how many weeks the current inventory amount might last.",
  direction: "above",
};
...
const InventoryWeeksOnHand = () => {
  ...
  const [inventoryWeeksOnHandReportData, setInventoryWeeksOnHandReportData] =
    useState([]);
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("inventoryItemName", {
        ...
        cell: ({ getValue }) => (
          <div className='w-full text-left'>{getValue()}</div>
        ),
        dataType: "string",
        size: 400,
        filterFn: "arrIncludesSome",
        ...
        footer: ({ table }) => (
          <div className='font-bold text-center'>
            {`Total = ${table
              .getFilteredRowModel()
              .rows.reduce(
                (acc, row) => acc + row.original.casesOnHandAtLastCount,
                0
              )
              .toFixed(2)}`}
          </div>
      ...
      columnHelper.accessor("estimatedValueOnHandNow", {
        ...
        cell: ({ getValue }) =>
          `$${getValue() !== 0 ? getValue().toFixed(2) : 0}`,
        dataType: "number",
        filterFn: "weakEquals",
        isFilterMenu: true,
      }),
  ...
  const fetchInventoryWeeksOnHandReport = async () => {
    try {
      ...
      const total = result.data
        .reduce((acc, item) => acc + item.estimatedValueOnHandNow, 0)
        .toFixed(2);
      ...
      <div className='w-[98%] mx-auto pageContainer'>
        ...
        <header className='optionsBar flex justify-between items-center mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
          <div className='flex items-center'>
            ...
            />
            <Run fetchData={fetchInventoryWeeksOnHandReport} />
          </div>
