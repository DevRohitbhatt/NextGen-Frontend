...
const ManagementSalesCoverage = () => {
          ...
          row.depth === 1 ? (
            <div className="mx-4">
              ...
              {formattingDataWithoutDollr(
                row.original?.projectedSalesPerCoverage
              )}
              %
            </div>
          ) : null
  ...
  useEffect(() => {
    const fetchLaborJobType = async () => {
      try {
        ...
        const newData = result?.data?.sort((a, b) =>
        ...
        const newData = result?.data
          ?.map((week) => ({
            ...
            week: `${dateFormat(
              week.weekStartDate,
              'mm/dd/yyyy'
            )} - ${dateFormat(week.weekEndDate, 'mm/dd/yyyy')}`,
            subRows: week.managementSalesCoverageReportUnitModels
              .filter(
                (unit) =>
                  unit.managementSalesCoverageReportDateModels.length > 0
              )
              .map((unit) => ({
                unitId: unit.unitId,
                unit: unitsAndAreasList?.units?.find(
                ...
                subRows: unit.managementSalesCoverageReportDateModels
                  .map((date) => ({
                    ...
                    projectedDailySales:
                      date.projectedDailySales === null
                        ? 0
                        : date.projectedDailySales,
                    actualDailySales:
                      date.actualDailySales === null
                        ? 0
                        : date.actualDailySales,
                    projectedManagementWork:
                      date.projectedManagementWork === null
                        ? 0
                        : date.projectedManagementWork,
                    actualManagementWork:
                      date.actualManagementWork === null
                        ? 0
                        : date.actualManagementWork,
                  }))
                  .filter(
                    (date, index, self) =>
  ...
  const formatPDFData = (data) => {
      ...
      row.subRows.map((subRow) => {
        return [
          ...
          },
          {
            value:
              `${formattingDataWithoutDollr(
                subRow.projectedSalesPerCoverage
              )}%` || '0.00%',
            cellType: 'text',
    ...
    const footerRow = [
      ...
      {
        value: `${formattingDataWithoutDollr(
          data[0].projectedSalesPerCoverage
        )}%`,
        cellType: 'text',
        columnName: 'Projected Sales % Coverage',
      },
      {
        value: `${formattingDataWithoutDollr(data[0].actualSalesPerCoverage)}%`,
        cellType: 'text',
        columnName: 'Actual Sales % Coverage',
      },
    ...
    const tableData = mgmtSalesCoverageData.flatMap((week) => {
      ...
      const weekRows = week.subRows.flatMap((unit) =>
        unit.subRows.map((date) => ({
          ...
          unit: unit.unit,
          date: date.date,
          projectedSalesPerCoverage: formattingDataWithoutDollr(
            date.projectedSalesPerCoverage
          ),
          actualSalesPerCoverage: formattingDataWithoutDollr(
            date.actualSalesPerCoverage
          ),
      ...
      weekRows.push({
        ...
        actualSalesPerCoverage:
          formattingDataWithoutDollr(weekTotalactualSales) + '%',
        scheduledShifts: '',
        actualShifts: '',
      });
