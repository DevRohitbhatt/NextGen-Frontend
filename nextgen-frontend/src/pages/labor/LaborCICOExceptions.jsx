...
const LaborCICOExceptions = () => {
	...
	const handlePDFClick = () => {
		const pdfData = {
			...
			body: [
				{
					type: 'table',
					widths: new Array((groupBy !== 'None' ? columns.slice(1) : columns).length).fill('auto'),
					dataTypes: (groupBy !== 'None' ? columns.slice(1) : columns).map((column) => column.dataType),
					data: {
						columnHeaders: (groupBy !== 'None' ? columns.slice(1) : columns).map((column) => column.header),
						rows: laborCICOExceptionsData.map((row) =>
							(groupBy !== 'None' ? columns.slice(1) : columns).map((column) => ({
	...
	const handleExcelClick = () => {
		const data = [
			{
				name: '',
				columns: (groupBy !== 'None' ? columns.slice(1) : columns).map((column) => ({
					name: column.header,
				})),
	...
	return (
		<>
			<div className='w-[98%] mx-auto'>
				...
				/>
				<h2 className='my-2 text-[18px] leading-tight text-left pageTitle'>Clock In - Clock Out Exceptions</h2>
