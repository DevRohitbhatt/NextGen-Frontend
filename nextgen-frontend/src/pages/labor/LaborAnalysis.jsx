...
const cancelApiObject = defineCancelApiObject({
...
const LaborAnalysis = () => {
  ...
  const fetchLaborAnalysisReport = async () => {
    const signal =
      cancelApiObject["laborAnalysis"].handleRequestCancellation().signal;
      ...
      const getData = {
        ...
        urlParams: {
          ...
          jobIdFilter:
            jobDescription === "All"
              ? " "
              : jobDetails.find((item) => item.description === jobDescription)
                  .jobId,
        },
        signal,
      };
              ...
              cell: ({ getValue, row }) => (
                <div
                  ...
                  onClick={() => {
                    if (
                      modalNames[
                        Object.keys(modalNames).find((key) =>
                          row.original.name.includes(key)
                        )
                      ]
                    ) {
                      handleModal(
                        row,
                        item,
                        result.data.scheduleSalesLaborHoursModels
                      );
                    }
                  }}
                >
  ...
  const handleModal = (row, item, data) => {
    ...
    modalColumns.unshift(
      columnHelper.accessor("name", { header: "Name", size: 150 })
    );

    const modalData = [
      row.original.name.includes("Scheduled Labor Percent") ||
      row.original.name.includes("Actual Labor Percent")
        ? filteredHours.reduce(
            (acc, hour) => {
              const hour24 =
                (parseInt(hour) % 12) + (hour.includes("PM") ? 12 : 0);
    ...
    return {
      ...
      rows: data.map((row) =>
        columns.map((column) => ({
          ...
          color: row[`${column.id}_color`] === "rgba(226, 240, 255, 255)"
		  ? ""
		  : row[`${column.id}_color`],
        }))
      ),
    };
  };
  ...
  const modalTable = (
    <TableHOC columns={modalColumns} data={laborAnalysisModalData} />
  );
