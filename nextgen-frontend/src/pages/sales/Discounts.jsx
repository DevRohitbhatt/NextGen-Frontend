...
const Discounts = () => {
  ...
  const memoizedColumns = useMemo(() => {
    const baseColumns = [
      ...
      columnHelper.accessor("discountedChecks", {
        ...
        footer: ({ table }) => (
          <div className='text-center'>
            {calculateFooterSum(table, "discountedChecks").toLocaleString(
              "en-US"
            )}
          </div>
        ),
      ...
      columnHelper.accessor("discountedItems", {
        ...
        footer: ({ table }) => (
          <div className='text-center'>
            {calculateFooterSum(table, "discountedItems")}
          </div>
        ),
      }),
      columnHelper.accessor("salesGenerated", {
        ...
        footer: ({ table }) => (
          <div className='text-center'>
            {formattingData(calculateFooterSum(table, "salesGenerated"))}
          </div>
        ),
      }),
      columnHelper.accessor("discountedTickets", {
        ...
        footer: ({ table }) => (
          <div className='text-center'>
            {formattingDataWithoutDollr(calculatePctFooter(table))}%
          </div>
        ),
      }),
    ];
    ...
    if (viewBy === "Date") {
      ...
      baseColumns.unshift(
        ...
        columnHelper.accessor("price", {
          ...
          footer: ({ table }) => (
            <div className='text-center'>
              {formattingData(calculateFooterSum(table, "price"))}
            </div>
          ),
          size: 80,
        }),
        columnHelper.accessor("amountDiscount", {
          ...
          footer: ({ table }) => (
            <div className='text-center'>
              {formattingData(calculateFooterSum(table, "amountDiscount"))}
            </div>
          ),
          size: 180,
        }),
  ...
  const calculatePctFooter = (table, weekId) => {
    ...
    if (viewBy === "Summary" && weekId !== "total") {
      totalSalesGenerated = table
        .getCoreRowModel()
        .rows.reduce((acc, row) => {
          const weekData = row.original.weeks.find(
            (week) => week[`salesGenerated_${weekId}`] !== undefined
          );
      ...
      discountAmount = table
        ...
        .rows.reduce((acc, row) => {
          const weekData = row.original.weeks.find(
            (week) => week[`discountAmount_${weekId}`] !== undefined
          );
      ...
      totalSalesGenerated = table.getCoreRowModel().rows.reduce((acc, row) => {
        const value = parseFloat(
          row.original["salesGenerated"] || row.original["totalSalesGenerated"]
        );
  ...
  const calculateFooterSum = (table, accessor) => {
    ...
    if (viewBy === "Summary" && !accessor.startsWith("total")) {
      footerSum = table
        ...
        .rows.reduce((acc, row) => {
          const weekData = row.original.weeks.find(
            (week) => week[accessor] !== undefined
          );
          return acc + (weekData ? parseFloat(weekData[accessor]) : 0);
        }, 0)
        .toFixed(
          accessor.startsWith("discountedItems") ||
            accessor.startsWith("discountedChecks")
            ? 0
            : 2
        );
    } else {
      footerSum = table
        ...
        .toFixed(
          accessor === "discountedChecks" || accessor === "discountedItems"
            ? 0
            : 2
        );
    }
    ...
    try {
      ...
      if (viewBy === "Summary") {
        const filterItems = discountTypesData.filter(
          (item) =>
            item.type ===
            discountTypeOptions.find((option) => option.name === discountType)
              ?.type
        );
        ...
        const filterData =
          discountType === "All-All Discounts"
            ...
            : result.data.filter((row) =>
                filterItems.find((item) => item.name === row.discountType)
              );
            ...
            : result.data.filter(
                (row) =>
                  ...
                  discountTypeOptions.find(
                    (option) => option.name === discountType
                  )?.typeName
              );
        ...
        const uniqueWeeks = Array.from(
          new Set(
            result.data.flatMap((item) => item.weeks.map((week) => week.weekId))
          )
        ).sort();
        ...
        const generatedColumns = [
            ...
            columns: [
              ...
              columnHelper.accessor("totaldiscountedTickets", {
                ...
                footer: ({ table }) => (
                  <div className='text-center'>
                    {formattingDataWithoutDollr(
                      calculatePctFooter(table, "total")
                    )}
                    %
                  </div>
                ),
          ...
          ...uniqueWeeks.map((weekId) =>
            columnHelper.group({
              header: `Week ${weekId} (${
                weeks.find((week) => week.weekId === weekId).WeekStartDate
              } - ${weeks.find((week) => week.weekId === weekId).WeekEndDate})`,
              columns: [
                columnHelper.accessor(
                  (row) => {
                    const weekData = row.weeks.find(
                ...
                columnHelper.accessor(
                  (row) => {
                    ...
                    return weekData
                      ? weekData[`discountedChecks_${weekId}`]
                      : 0;
                  },
                  {
                ...
                columnHelper.accessor(
                  (row) => {
                    ...
                    return weekData
                      ? weekData[`discountedTickets_${weekId}`]
                      : 0;
                  },
                  {
                    ...
                    footer: ({ table }) => (
                      <div className='text-center'>
                        {formattingDataWithoutDollr(
                          calculatePctFooter(table, weekId)
                        )}
                        %
                      </div>
                    ),
        ...
        handleGroupByChange(
          viewBy === "Summary" || viewBy === "Detail" ? groupBy : viewBy,
          generatedColumns
        );
      } else {
  ...
  const handleGroupByChange = (option, columnsPassed) => {
    ...
    if (option !== "None") {
      const firstNonGroupByColumnIndex = newColumns.findIndex(
        (col) => !col.groupBy
      );
      ...
      if (firstNonGroupByColumnIndex !== -1) {
        const updateColumn = (column) => ({
          ...
          cell: ({ row, getValue }) => {
            ...
            return row.getCanExpand() ? (
              ...
              <div
                className={`${
                  column.header === "Discount Type" ? "text-left px-6" : ""
                }`}
              >
                {getValue()}
              </div>
        ...
        if (viewBy === "Summary") {
          newColumns[firstNonGroupByColumnIndex].columns[
            firstNonGroupByColumnIndex - 1
          ] = updateColumn(
            newColumns[firstNonGroupByColumnIndex].columns[
              firstNonGroupByColumnIndex - 1
            ]
          ...
          newColumns[firstNonGroupByColumnIndex] = updateColumn(
            newColumns[firstNonGroupByColumnIndex]
          );
        }
      }
    } else {
              ...
              rows: data.slice(i, i + rowsPerTable).map((row) =>
                columnChunk.map((column) => {
                  ...
                  let value = isMetricColumn
                    ? row.weeks?.find((week) => week[column.id])?.[column.id] ??
                      0
                    : row[column.id] ?? 0;
  ...
  const Table = (
    <TableHOC
      columns={viewBy === "Summary" ? summaryColumns : columns}
      data={discountsData}
      isFooter={true}
    />
  );
          ...
          <div className='flex items-center'>
            ...
            <div className='mx-2 w-52 discount-selector'>
              <Dropdown
                title='Discounts'
                options={
                  isDiscountTypesLoading
                    ? [{ name: "Loading..." }]
                    : discountTypeOptions
                }
                selectedOption={
                  isDiscountTypesLoading ? "Loading..." : discountType
                }
