...
const SalesSummary = () => {
		...
		() => [
			...
			},
			{
				...
				footer: ({ table }) => {
					let qtyItem = table
						.getCoreRowModel()
						.rows.reduce((sum, item) => sum + parseFloat(item.original.quantityItems), 0);
			...
			},
			{
				...
				footer: ({ table }) => {
					let quantityModifiers = table
						.getCoreRowModel()
						.rows.reduce((sum, item) => sum + parseFloat(item.original.quantityModifiers), 0);
			...
			},
			{
				...
				footer: ({ table }) => {
					let salesNet = table
						.getCoreRowModel()
						.rows.reduce((sum, item) => sum + parseFloat(item.original.salesNet), 0);
					return <div className='w-full text-center'>{formattingData(salesNet)}</div>;
				},
			},
			{
		...
		() => [
			...
			},
			{
				...
				footer: ({ table }) => {
					let quantity = table
						.getCoreRowModel()
						.rows.reduce((sum, item) => sum + parseFloat(item.original.quantity), 0);
			...
			},
			{
				...
				footer: ({ table }) => {
					let amount = table
						.getCoreRowModel()
						.rows.reduce((sum, item) => sum + parseFloat(item.original.amount), 0);
					return <div className='w-full text-center'>{formattingData(amount)}</div>;
				},
			},
			{
				accessorKey: 'tip',
			...
			},
			{
				accessorKey: 'total',
				header: 'Total',
				cell: (info) => formattingData(info.row.original.amount + info.row.original.tip),
				footer: ({ table }) => {
					let total = table
						.getCoreRowModel()
						.rows.reduce(
					...
					return <div className='w-full text-center'>{formattingData(total)}</div>;
				},
			},
			{
				accessorKey: 'percentOfTotal',
	...
	const viewActivityData = React.useMemo(() => {
		const data =
			salesSummaryData?.viewActivityDaily && salesSummaryData?.viewActivityDaily.length > 0
		...
		const splitIndex = transformedData.findIndex((item) => item.label === 'Cash OverShort');
			...
			if (result?.data && result.data.categorySummary.length > 0) {
				...
				let viewActivityDaily = totalOfViewActivity.map((item) => {
					let activity = {
						...
						laborHours: formattingDataWithoutDollr(item?.laborVariableHours + item?.laborSalary),
						transactions: formattingDataWithoutDollr(item.transactions.toFixed(0)) + '',
						nrsTotalOpen: item.nrsTotalOpen + '',
						laborPercent:
							item?.laborVariable == 0
								? 0.0
								: getLabourPercent(item?.laborVariable, item?.laborSalary, item?.salesNet).toFixed(2),
					};
					return activity;
				});
	...
	const handlePDFClick = () => {
		const metaInfo = {
			groupOrUnitAccessName,
			selectedUnitName,
			selectedDate: `${dateFormat(selectedFromDate, 'mm-dd-yyyy')} - ${dateFormat(selectedToDate, 'mm-dd-yyyy')}`,
		};
	...
	const prepareExcelData = (salesSummaryData) => {
		...
		const grossSalesSection = {
			...
			data: [
				['Gross Sales', `${formattingData(salesSummaryData.viewActivityDaily[0]?.salesGross)}`],
				['Net Sales', `${formattingData(salesSummaryData.viewActivityDaily[0]?.salesNet)}`],
				['Tax', `${formattingData(salesSummaryData.viewActivityDaily[0]?.salesTax)}`],
				['Comps', `${formattingData(salesSummaryData.viewActivityDaily[0]?.comps)}`],
				['Promotions', `${formattingData(salesSummaryData.viewActivityDaily[0]?.promo)}`],
				['Refunds', `${formattingData(salesSummaryData.viewActivityDaily[0]?.refunds)}`],
				['Voids', `${formattingData(salesSummaryData.viewActivityDaily[0]?.voids)}`],
				['Sales Non Cash', `${formattingData(salesSummaryData.viewActivityDaily[0]?.planSalesNet)}`],
				['Deposits', `${formattingData(salesSummaryData.viewActivityDaily[0]?.deposits)}`],
				['Cash OverShort', `${formattingData(salesSummaryData.viewActivityDaily[0]?.cashOverShort)}`],
				['Order Count', salesSummaryData.viewActivityDaily[0]?.transactions || '0'],
				...
				['Gift Cards Redeemed', `$${salesSummaryData.viewActivityDaily[0]?.giftCertificate?.toFixed(2)}`],
				['Gift Cards Sold', `$${salesSummaryData.viewActivityDaily[0]?.giftCertificatesSold?.toFixed(2)}`],
			],
		};
		...
		categoryData[categoryData.length] = {
			...
			quantityItems: valueFormatewithoutDecimal(
				categoryData.reduce((sum, item) => sum + parseFloat(item.quantityItems), 0)
			),
			quantityModifiers: valueFormatewithoutDecimal(
				categoryData.reduce((sum, item) => sum + parseFloat(item.quantityModifiers), 0)
			),
			salesNet: categoryData.reduce((sum, item) => sum + parseFloat(item.salesNet), 0),
			percentOfTotal: 1,
		};
		...
		paymentData[paymentData.length] = {
			name: 'Total',
			quantity: paymentData.reduce((sum, item) => sum + parseFloat(item.quantity || 0), 0),
			amount: paymentData.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0),
			tip: paymentData.reduce((sum, item) => sum + parseFloat(item.tip || 0), 0),
		...
		discountData[discountData.length] = {
			typeItemName: 'Total',
			quantityTickets: discountData.reduce((sum, item) => sum + parseFloat(item.quantityTickets || 0), 0),
			amountDiscount: discountData.reduce((sum, item) => sum + parseFloat(item.amountDiscount || 0), 0),
		...
		const discountsSection = {
			name: 'Discounts',
			columns: [{ name: 'Name' }, { name: 'Quantity' }, { name: 'Total' }, { name: 'Percent' }],
			data: discountData.map((item, index) => [
				...
				`${
					index !== discountData.length - 1
						? formattingDataWithoutDollr(item.amountDiscountPercentOfTotal * 100) + '%'
						: ''
				}`,
			]),
		};
	...
	const handleExportToExcel = (salesSummaryData, filename, spreadSheetTitle, date, unitName) => {
	...
	return (
		<div className='w-[98%] mx-auto pageContainer'>
			...
			/>
			<h2 className='my-2 text-[18px] leading-tight text-left pageTitle'>Sales Summary</h2>
			<header className='optionsBar flex justify-between items-center mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
				...
				<div>
					<ExportOptions
						...
						includeCSV={true}
						handleCSVClick={() => handleExportToCSV(salesSummaryData, 'Sales_Summary_Report')}
						includeExcel={true}
						handleExcelClick={() => {
							handleExportToExcel(
								salesSummaryData,
			...
			{isLoading ? (
				...
				<>
					<div className='py-4 mb-0 rounded-lg all'>
						<div className='paged-table '>
							<div className='flex flex-row gap-4 main-container'>
								<div className='w-1/2'>
									<TableHOC
					...
					{Object.keys(salesSummaryData).length > 0 && (
						<div className=' paged-table rounded-lg py-4 mt-[-20px] categorySales'>
							<h3 className='mb-4 text-lg font-semibold'>Category Sales</h3>
							<TableHOC
					...
					<div className=' paged-table rounded-lg py-4 mt-[-20px] Discounts'>
						<h3 className='mb-4 text-lg font-semibold'>Discounts</h3>
						<TableHOC
