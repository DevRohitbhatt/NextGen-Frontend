...
const tooltips = {
  ...
  avgItemQuantity:
    "The average quantity sold per day during the selected date range.",
    ...
    try {
      ...
      if (viewValue === 0) {
        newData = result.data.menuItemSoldModifierReportModels
          .map((category) => ({
            ...
            totalItemQuantity:
              category.menuItemSoldModifierModels[0].totalItemQuantity,
          ...
          .sort((a, b) =>
            a.category
              .slice(0, 1)
              .localeCompare(b.category.slice(0, 1), undefined, {
                numeric: true,
              })
          );
      } else if (viewValue === 1) {
        newData = result.data.menuItemSoldModifierReportModels
          .map((category) => ({
            ...
            subRows: category.menuItemSoldModifierUnitReportModels.map(
              (unit) => ({
                totalItemQuantity:
                  unit.menuItemSoldModifierModels[0].totalItemQuantity,
                ...
                subRows: unit.menuItemSoldModifierModels.map((item) => ({
                  ...
                  quant:
                    item.modQuantity === 0
                      ...
                      : item.modQuantity,
                  modItemID:
                    item.modItemID === 0 ? item.itemId : item.modItemID,
                  modifierDisplayName: item.modifierDisplayName,
      ...
      setColumns([
        ...
        columnHelper.accessor("modItemFrequency", {
          ...
          dataType: "string",
          cell: ({ getValue }) =>
            getValue() ? `${(getValue() * 100).toFixed(2)}%` : "",
        }),
      ]);
