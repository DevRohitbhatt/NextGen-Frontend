...
const ItemsSoldWithModifiers = () => {
		...
		try {
			...
			if (viewValue === 0) {
				newData = result.data.menuItemSoldModifierReportModels
					...
					.sort((a, b) =>
						a.category.slice(0, 1).localeCompare(b.category.slice(0, 1), undefined, {
							numeric: true,
						})
					);
			} else if (viewValue === 1) {
		...
		return viewValue === 0
			? {
					...
					rows: data.map((subRow) => [
						{
							value: subRow.modItemID,
							cellType: 'string',
							columnName: 'Modifier Item #',
						},
						{
						...
						{
							value: `${formattingDataWithoutDollr(subRow.modItemFrequency)}%`,
							cellType: 'number',
				...
				? menuItemSoldData.flatMap((category) =>
						category.subRows.map((item) =>
							[
								...
								item.quant,
								`${formattingDataWithoutDollr(item.modItemFrequency)}%`,
							].join(',')
						)
				  )
				: menuItemSoldData.flatMap((category) =>
	...
	const handleExcelClick = () => {
					...
					viewValue === 0
						? menuItemSoldData.flatMap((category) =>
								category.subRows.map((item) => ({
									...
									Quantity: item.quant,
									'Modifier Usage Frequency': `${formattingDataWithoutDollr(item.modItemFrequency)}%`,
								}))
						  )
						: menuItemSoldData.flatMap((category) =>
								category.subRows.flatMap((unit) =>
									unit.subRows.map((item) => ({
										...
										'Modifier Usage Frequency': `${formattingDataWithoutDollr(
											item.modItemFrequency
										)}%`,
									}))
								)
						  ),
		...
		const spreadSheetTitle = 'Menu Item Sold | Items Sold With Modifiers';
			...
			<header className='optionsBar mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
				<div className='flex items-center justify-between space-x-3'>
					<div className='flex items-center space-x-1'>
						...
						<Run fetchData={fetchItemsSoldWithModifiersData} />
					</div>
