...
const ItemsSoldByHour = () => {
			...
			...(itemValue === 1
				? [
						...
						columnHelper.accessor('usageCases', {
							...
							header: 'Usage Cases',
							dataType: 'percent',
							cell: (info) => `${formattingDataWithoutDollr(info.getValue())}%` || '',
						}),
		...
		const csvData = menuItemSoldData.flatMap((item) =>
			[
				...
				...(itemValue === 1
					? [
							item.caseUnitName,
							`${formattingDataWithoutDollr(item.usageCases)}%`,
							item.countDisplayUnitName,
							`${formattingDataWithoutDollr(item.usageCountDisplayUnits)}%`,
					  ]
					: []),
			].join(',')
		);
	...
	const handleExcelClick = () => {
		const data = [
			{
				...
				data: menuItemSoldData.flatMap((item) => ({
					...
					'# Sold': item.sold,
					'Item Sales': formattingData(item.discPrice),
					...(itemValue === 1
						? {
								'Case Unit Name': item.caseUnitName,
								'Usage Cases': `${formattingDataWithoutDollr(item.usageCases)}%`,
								'Count Name': item.countDisplayUnitName,
								'Use Count': `${formattingDataWithoutDollr(item.usageCountDisplayUnits)}%`,
						  }
						: {}),
				})),
			},
		...
		const spreadSheetTitle = 'Menu Item Sold | Items Sold By Hour';
			...
			<header className='optionsBar mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
				<div className='flex items-center justify-between space-x-3'>
					<div className='flex items-center space-x-1'>
						...
						<Run fetchData={fetchSoldByHourData} />
					</div>
