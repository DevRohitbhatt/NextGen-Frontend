		...
		return viewValue === 0
			? {
					...
					rows: data.map((subRow) => [
						...
						{
							value: `${formattingDataWithoutDollr(subRow.itemSoldPct)}%`,
							cellType: 'number',
						...
						{
							value: formattingData(subRow.discPrice_Avg),
							cellType: 'number',
				...
				? menuItemSoldData.flatMap((row) =>
						row.subRows.map((subRow) =>
							[
								...
								subRow.description,
								subRow.quant,
								formattingData(subRow.discPrice),
								`${formattingDataWithoutDollr(subRow.itemSoldPct)}%`,
								subRow.quantity_Avg,
								formattingData(subRow.discPrice_Avg),
							].join(',')
						)
				  )
				: viewValue === 1
						...
						category.subRows.flatMap((unit) =>
							unit.subRows.map((item) =>
								[
									...
									item.description,
									item.quant,
									formattingData(item.discPrice),
									`${formattingDataWithoutDollr(item.itemSoldPct)}%`,
									item.quantity_Avg,
									formattingData(item.discPrice_Avg),
								].join(',')
							)
						)
				  )
					...
					viewValue === 0
						? menuItemSoldData.flatMap((category) =>
								category.subRows.map((item) => ({
									...
									Description: item.description,
									Quantity: item.quant,
									Amount: formattingData(item.discPrice),
									'Item Sold %': `${formattingDataWithoutDollr(item.itemSoldPct)}%`,
									'Average Item Quantity': item.quantity_Avg,
									'Average Item Amount': formattingData(item.discPrice_Avg),
								}))
						  )
						: viewValue === 1
						? menuItemSoldData.flatMap((category) =>
			...
			<header className='optionsBar mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
				<div className='flex items-center justify-between space-x-3'>
					<div className='flex items-center space-x-1'>
						...
						<Run fetchData={fetchItemsSoldTotalsData} />
					</div>
				...
				<div className='flex mt-2'>
					<div className='mt-2 view-selector'>
						<label className='block ml-2 mb-1 mt-[-12px] text-[16px] font-semibold'>View</label>
					...
					<div className='pl-2 mt-2 sale-selector'>
						<label className='block ml-2 mb-1 mt-[-12px]  font-semibold text-[16px]'>Sales</label>
						<div className='p-2 border-2 border-solid rounded-[1.5rem] checkbox-group hover:border-primary'>
							<div className='flex flex-row space-x-6'>
								<div className='flex items-center cursor-pointer'>
									<input
