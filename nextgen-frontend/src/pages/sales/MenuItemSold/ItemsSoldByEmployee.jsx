...
const ItemsSoldByEmployee = () => {
	...
	const generatedColumns = [
		columnHelper.accessor('unitName', {
			id: 'unitName',
			header: <div className='w-full text-left'>Unit</div>,
			dataType: 'number',
		...
		columnHelper.accessor('employeeCoversPercent', {
			...
			cell: (info) => {
				...
				return value != null ? `${formattingDataWithoutDollr(value)}%` : '';
			},
		}),
	];
							...
							row.depth < selectedGroupByColumns.length
								? `${
										columns.find((col) => col.id === selectedGroupByColumns[row.depth])?.header
											.props.children
								  }: ${row.original[selectedGroupByColumns[row.depth]]} (Count: ${
	...
	const handleCSVClick = () => {
		...
		const csvData = menuItemSoldData.flatMap((item) =>
			[
				...
				item.employeeCovers,
				`${formattingDataWithoutDollr(item.employeeCoversPercent)}%`,
			].join(',')
		);
		...
		const data = [
			{
				...
				data: menuItemSoldData.flatMap((item) => ({
					...
					'% of Guests': `${formattingDataWithoutDollr(item.employeeCoversPercent)}%`,
				})),
			},
		];
			...
			<header className='optionsBar mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
				<div className='flex items-center justify-between space-x-3'>
					<div className='flex items-center space-x-1'>
						...
						<Run fetchData={fetchSoldByEmpData} />
					</div>
