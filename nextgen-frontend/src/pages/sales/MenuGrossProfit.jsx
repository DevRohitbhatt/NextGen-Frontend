	...
	const fetchMenuGrossProfitData = async (memberId) => {
		try {
			...
			const newData = result.data.menuGrossSalesCodeModels.map((item) => ({
				...
				netProfit: result.data.menuGrossProfitFooterModel.netProfit.slice(1).replace(/[¤]/g, ''),
			}));
			...
			body: [
				{
					...
					data: {
						...
						rows: menuGrossProfitData.map((row) => [
							...
							{
								value: formattingData(row.itemPrice),
								cellType: 'string',
								columnName: 'Item Price',
							},
							{
								value: formattingData(row.recipeCost),
								cellType: 'string',
								columnName: 'Recipe Cost',
							},
							{
								value: `${formattingDataWithoutDollr(row.costper)}%`,
								cellType: 'string',
								columnName: 'Cost %',
							},
							{
							...
							{
								value: row.itemSales === 0 ? '0 ' : formattingData(row.itemSales),
								cellType: 'string',
								columnName: 'Item Sales',
							},
							{
								value: formattingData(row.grossProfit),
								cellType: 'string',
								columnName: 'Gross Profit',
							},
							{
								value: `${formattingDataWithoutDollr(row.grossProfitper)}%`,
								cellType: 'string',
								columnName: 'Gross Profit %',
							},
						]),
		...
		const data = [
			{
				...
				data: menuGrossProfitData.map((row) => [
					...
					row.itemID,
					row.itemName,
					formattingData(row.itemPrice),
					formattingData(row.recipeCost),
					`${formattingDataWithoutDollr(row.costper)}%`,
					row.quantitySold,
					formattingData(row.itemSales),
					formattingData(row.grossProfit),
					`${formattingDataWithoutDollr(row.grossProfitper)}%`,
				]),
			},
		];
