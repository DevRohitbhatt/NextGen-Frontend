...
const MenuGrossProfit = () => {
  ...
  const [showItemsWithNoRecipeCost, setShowItemsWithNoRecipeCost] =
    useState(false);
        ...
        return (
          <div className='h-10'>
            ...
            >
              ...
              <div>
                Total Gross Sales: $
                {table.getCoreRowModel().rows[0].original.totalGrossSales}
              </div>
              <div>
                Gross Food Cost: $
                {table.getCoreRowModel().rows[0].original.grossFoodCost}
              </div>
              <div>
                Gross Profit: $
                {table.getCoreRowModel().rows[0].original.grossProfitFooter}
              </div>
              <div>
                Total Net Sales: $
                {table.getCoreRowModel().rows[0].original.totalNetSales}
              </div>
              <div>
                Net Food Cost: $
                {table.getCoreRowModel().rows[0].original.netFoodCost}
              </div>
              <div>
                Net Profit: $
                {table.getCoreRowModel().rows[0].original.netProfit}
              </div>
            </div>
          </div>
        );
  ...
  const fetchMenuGrossProfitData = async (memberId) => {
    try {
      ...
      const newData = result.data.menuGrossSalesCodeModels.map((item) => ({
        ...
        totalGrossSales:
          result.data.menuGrossProfitFooterModel.totalGrossSales.slice(1),
        grossFoodCost:
          result.data.menuGrossProfitFooterModel.grossFoodCost.slice(1),
        grossProfitFooter:
          result.data.menuGrossProfitFooterModel.grossProfit.slice(1),
        totalNetSales:
          result.data.menuGrossProfitFooterModel.totalNetSales.slice(1),
        netFoodCost:
          result.data.menuGrossProfitFooterModel.netFoodCost.slice(1),
      ...
      let filteredData = newData.filter((item) =>
        categoriesTemp.includes(item.category)
      );
  ...
  const updateSelectedCategories = (newData) => {
    if (!selectedCategories.length) {
      ...
      const newCategories = [
        ...new Set(newData.map((item) => item.category)),
      ].sort();
    ...
    //if there are existing selected categories, check if they are still available, if not then remove. Then and add any new ones
      ...
      const updatedCategories = selectedCategories.filter((category) =>
  ...
  const handleRecipeInfoModal = async (e) => {
    ...
    try {
      ...
      console.log(
        "Error getting Menu Gross Profit Recipe Information data: ",
        e
      );
    }
  };
  ...
  const handleExcelClick = () => {
    ...
    exportToExcel(
      ...
      spreadSheetTitle,
      date,
      `${selectedUnitName}  |  Generated on ${dateFormat(
        new Date(),
        "mm/dd/yyyy"
      )}`
    );
      ...
      <div className='w-[98%] mx-auto'>
        ...
        <header className='optionsBar flex justify-between items-center mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
          <div className='flex items-center'>
            ...
            <Run fetchData={() => fetchMenuGrossProfitData(selectedUnit)} />
          </div>
          <div>
            <ExportOptions
              includePDF={true}
        ...
        <div className='flex gap-4 '>
          <div className='flex items-center rounded'>
            ...
            />
            <label className='text-[14px] font-medium ms-2'>
              Show Items With Sales of $0
            </label>
          </div>
          <div className='flex items-center rounded'>
            <input
              ...
              onClick={() =>
                setShowItemsWithNoRecipeCost(!showItemsWithNoRecipeCost)
              }
            />
            <label className='text-[14px]  font-medium ms-2'>
              Show Items With No Recipe Cost
            </label>
          </div>
          <div className='flex items-center rounded'>
            <input
            ...
            <div className='p-4 w-[32rem] space-y-4 '>
              ...
              <div className='flex flex-col space-y-2 overflow-auto max-h-96 tableHOC'>
                {categories.map((category) => (
                  <div
                    key={category}
                    className='flex items-center rounded text-[14px]'
                  >
                    <input
                      type='checkbox'
                    ...
                    />
                    <label className=' font-medium ms-2 text-[14px]'>
                      {category}
                    </label>
                  </div>
                ))}
