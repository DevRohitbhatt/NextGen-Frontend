  ...
  const getSalesData = async () => {
    ...
    try {
      ...
      if (result?.data) {
        ...
        console.log(a,"<")
        let viewActivityDaily = a.map(
          (item) => {
            let activity = {
              ...item,
              orderAverage: formattingData(item?.salesNet / item?.transactions),
              laborCost: formattingData(
                item?.laborVariable + item?.laborSalary
              ),
...
//   }, [selectedUnit]);
  ...
  const prepareExcelData = (salesSummaryData) => {
    ...
    const grossSalesSection = {
      ...
      data: [
        ["Gross Sales", `$${salesSummaryData.viewActivityDaily[0]?.salesGross?.toFixed(2)}`],
        ["Net Sales", `$${salesSummaryData.viewActivityDaily[0]?.salesNet?.toFixed(2)}`],
        ["Tax", `$${salesSummaryData.viewActivityDaily[0]?.salesTax?.toFixed(2)}`],
        ["Comps", `$${salesSummaryData.viewActivityDaily[0]?.comps?.toFixed(2)}`],
        ["Promotions", `$${salesSummaryData.viewActivityDaily[0]?.promo?.toFixed(2)}`],
        ["Refunds", `$${salesSummaryData.viewActivityDaily[0]?.refunds?.toFixed(2)}`],
        ["Voids", `$${salesSummaryData.viewActivityDaily[0]?.voids?.toFixed(2)}`],
        ["Sales Non Cash", `$${salesSummaryData.viewActivityDaily[0]?.salesFood?.toFixed(2)}`],
        ["Deposits", `$${salesSummaryData.viewActivityDaily[0]?.deposits?.toFixed(2)}`],
        ["Cash OverShort", `$${salesSummaryData.viewActivityDaily[0]?.cashOverShort?.toFixed(2)}`],
        ...
        [
          "Order Average",
          `$${(salesSummaryData.viewActivityDaily[0]?.salesNet / salesSummaryData.viewActivityDaily[0]?.transactions).toFixed(2)}`,
        ],
        [
          "Labor Cost",
          `$${(salesSummaryData.viewActivityDaily[0]?.laborVariable + salesSummaryData.viewActivityDaily[0]?.laborSalary).toFixed(2)}`,
        ],
        [
          "Labor Hours",
          `$${(salesSummaryData.viewActivityDaily[0]?.laborVariableHours + salesSummaryData.viewActivityDaily[0]?.laborSalaryHours).toFixed(2)}`,
        ],
        [
          "Labor Percent",
          `${(
            ((salesSummaryData.viewActivityDaily[0]?.laborVariable + salesSummaryData.viewActivityDaily[0]?.laborSalary) /
    ...
    <div className="w-[85%] mx-auto">
      ...
      <header className="optionsBar flex justify-between items-center mb-2 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]">
        <div className="flex items-center">
          ...
          <div className="run-button" onClick={() => {getSalesData();}}>
        ...
        <div>
          <ExportOptions
            ...
            includeCSV={true}
            handleCSVClick={()=>handleExportToCSV(
                salesSummrayData,
                "Sales_Summary_Report"
              )
              }
            includeExcel={true}
            handleExcelClick={()=>{handleExportToExcel(
                salesSummrayData,
                "Sales_Summary_Report",
