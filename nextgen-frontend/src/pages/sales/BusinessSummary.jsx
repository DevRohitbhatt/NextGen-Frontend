...
const BusinessSummary = () => {
  ...
  const [useGrossSalesInBusinessSummary, setUseGrossSalesInBusinessSummary] =
    useState(false);
    ...
    if (companyID) {
      const fetchCompanySettings = async () => {
        try {
          ...
          const status = result.data.find(
            (setting) => setting.name === "UseGrossSalesInBusinessSummary"
          )
            ? true
            : false;
      ...
      if (result.data.length === 0) {
        ...
        newData.forEach((data) => {
          if (
            ...
            const variableLabor = newData.find(
              (item) => item.description === "Variable Labor"
            );
            const foodCostPct = newData.find(
              (item) => item.description === "Food Cost %"
            );
            const netSales = newData.find(
              (item) => item.description === salesType
            );
            const transactions = newData.find(
              (item) => item.description === "Transactions"
            );
            const netSalesTotal = parseFloat(
              netSales.total.replace(/[$,]/g, "")
            );
            const transactionTotal = parseFloat(
              transactions.total.replace(/[$,]/g, "")
            );
            const variableLaborTotal = parseFloat(
              String(variableLabor.total).replace(/[$,]/g, "")
            );
            ...
            if (
              ...
              data.total =
                ((variableLaborTotal / netSalesTotal) * 100).toLocaleString(
                  "en-US",
                  {
                    maximumFractionDigits: 2,
                    minimumFractionDigits: 2,
              ...
              data.total =
                (
                  ...
                  Object.keys(data).filter(
                    (key) => !["description", "total"].includes(key)
                  ).length
      ...
      <div className='w-[98%] mx-auto'>
        ...
        <header className='optionsBar flex justify-between items-center mb-0 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
          <div className='flex items-center'>
            ...
            <Run fetchData={fetchBusinessSummaryReport} />
          </div>
          <div>
            <ExportOptions
              includePDF={true}
