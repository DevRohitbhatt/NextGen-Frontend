...
const BrumitDWP = () => {
  ...
  const fetchBrumitDWP = async () => {
    const signal =
      cancelApiObject["brumitDWP"].handleRequestCancellation().signal;
    try {
      setIsLoading(true);
      setIsError(false);
      const getData = {
        ...
        urlParams: {
          ...
          Options: reportTypeOptions.find(
            (option) => option.name === reportType
          ).value,
        },
        signal,
      };
      ...
      if (result.data.length === 0) {
        ...
        const newData = result.data.map((row) => {
          if (
            ...
            return {
              ...Object.keys(row.unitGroups).reduce((acc, key) => {
                if (row.itemName === "Net Sales") {
                  ...
                  acc[key] = `${formattingDataWithoutDollr(
                    row.unitGroups[key] * 100
                  )}%`;
                }
                return acc;
              }, {}),
            ...
            ["LY Sales", "TY Trans", "LY Trans"].includes(row.itemName)
          ) {
            return {
              ...
              ...Object.keys(row.unitGroups).reduce((acc, key) => {
                if (row.itemName === "LY Sales") {
                  acc[key] =
                    row.unitGroups[key] < 0
                      ? `-$${Math.round(row.unitGroups[key]).toLocaleString(
                          "en-US"
                        )}`
                      : `$${Math.round(row.unitGroups[key]).toLocaleString(
                          "en-US"
                  ...
                  acc[key] = formattingDataWithout(
                    parseInt(row.unitGroups[key])
                  );
                }
                return acc;
              }, {}),
            ...
            .map((key) =>
              columnHelper.accessor(key, {
                ...
                cell: ({ getValue }) => (
                  <div
                    className={`${
                      String(getValue())?.includes("-") ? "text-[#D43F3A]" : ""
                    }`}
                  >
                    {getValue()}
                  </div>
        ...
        data: brumitDWPData.flatMap((row) =>
          (row.subRows || [row]).map((subRow) =>
            columns.map((column) => ({
              ...
              fontColor:
                parseFloat(subRow[column.id]) < 0 ? "rgba(212, 63, 58, 1)" : "",
            }))
          )
        ),
      },
