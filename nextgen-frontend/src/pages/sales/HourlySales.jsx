...
const HourlySales = () => {
    ...
    try {
      ...
      const getData = {
        ...
        urlParams: {
          ...
          reportType:
            reportType === "Hour and Day" ? 1 : reportType === "Unit" ? 2 : 3,
          sumType:
            viewBy === "Hour"
              ? "HOUR"
              : viewBy === "Half-Hour"
      ...
      const newData = result.data.map((data) => {
        const updatedData = {
          ...
          Avg:
            data.Total /
            Object.keys(data).filter((key) => /^\d{2}\/\d{2}\/\d{4}$/.test(key))
              .length,
        };
  ...
  const handleExcelClick = () => {
    ...
    const filename = `HourlySales_${selectedUnitName}_${dateFormat(
      selectedFromDate,
      "mm-dd-yyyy"
  ...
  const Table = (
    <TableHOC columns={columns} data={hourlySalesData} isFooter={true} />
  );
  return (
      ...
      <header className='optionsBar flex justify-between items-center mb-0 rounded-2xl p-4 shadow-[0px_3px_20px_-10px_rgba(0,_0,_0,_0.5)]'>
        <div className='flex flex-col '>
          <div className='flex items-center space-x-1'>
            ...
            <Run fetchData={fetchHourlySalesReport} />
          </div>
