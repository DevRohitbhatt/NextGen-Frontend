import PdfBuilder from '../components/exportOptions/PdfBuilder';
...
export const updateDefaultSafetyFactor = (
	...
	updatedSuggestedOrderData = calculateSuggestedQuantities(
		forecastTotal,
		updatedSuggestedOrderData,
		orderLimits,
		true
	);
...
export const calculateSuggestedQuantities = (forecastTotal, suggestedOrderData, orderLimits, recalculate = false) => {
	const updatedSuggestedOrderData = suggestedOrderData.rows.map((detail) => {
		return {
			name: detail.name,
			suggestedOrderItem: detail.suggestedOrderItem.map((inventoryItem) => {
				var orderLimit = orderLimits.find(
					(orderLimit) => orderLimit.qsrInventoryItemID === inventoryItem.qsrInventoryItemID
				);
				return {
					...inventoryItem,
					vendorItems: inventoryItem.vendorItems.map((vendorItem) => {
						if (
							(vendorItem.suggestedQty !== 0 ||
								vendorItem.onHandQty !== 0 ||
								vendorItem.orderQty !== 0) &&
							!recalculate
						) {
							var extendedPrice = calculateExtendedPrice(
								vendorItem.orderQty,
								vendorItem.latestInvoicePrice
							);
							return {
								...vendorItem,
...
export const handleEdit = async (
	...
	const updatedSuggestedOrderItem = node.suggestedOrderItem.map((childNode) => {
		let returntItem = {
			...childNode,
			vendorItems: childNode.vendorItems.map((vendorItem) => {
	...
	if (['safetyFactor', 'onHandQty', 'orderQty', 'extendedPrice'].includes(field)) {
		onEdit(updatedNode);
	}
...
export const onSearch = (searchTerm, data, setFilteredData, setExpandedNodes) => {
	...
	const filteredData = data.rows.map((node) => {
		const updatedSuggestedOrderItem = node.suggestedOrderItem.map((item) => {
			let isHidden = false;
			if (item.invItemDescription.toLowerCase().includes(searchTerm.toLowerCase())) {
